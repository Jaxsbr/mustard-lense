...
function displayLabel(record: PanelRecord): string {
  return record.title?.trim() ? record.title : truncate(record.text)
}
...
export function TodoListItem({ record }: { record: PanelRecord }) {
  ...
  return (
    <div className="list-item list-item--todo" data-testid="list-item-todo">
      ...
      <span className="list-text">{displayLabel(record)}</span>
...
export function ListItem({ record, type }: { record: PanelRecord; type: string }) {
  switch (type) {
    ...
    default: return <div className="list-item"><span className="list-text">{displayLabel(record)}</span></div>
  }
}
