...
- [x] `npm run smoke:basic` invokes the real `claude` CLI in basic mode with a prompt asking it to report its permission/restriction mode [US-L4]
- [x] `npm run smoke:admin` invokes the real `claude` CLI in admin mode with a prompt asking it to report its permission/restriction mode [US-L4]
- [x] `smoke:basic` test output contains indication of restricted/standard mode (verified via string contains check) [US-L4]
